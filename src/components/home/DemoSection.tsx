...
// Time on the full-width concept slide before the deck "advances".
const INTRO_MS = 3600;
// Time on the split coding-question view before the demo loops.
...
export const DemoSection = React.forwardRef<HTMLElement, Props>(
  ({ isVisible }, ref) => {
    ...
    useEffect(() => {
      ...
      setPhase("intro");
      const advance = setTimeout(() => setPhase("question"), INTRO_MS);
      const loop = setTimeout(
        () => setCycle((c) => c + 1),
        INTRO_MS + QUESTION_MS
      );
      return () => {
        clearTimeout(advance);
        clearTimeout(loop);
      };
    }, [isVisible, cycle, reduceMotion]);

    const split = phase === "question";
            ...
            <div className="relative bg-[#1a1a1a] rounded-2xl shadow-2xl border-2 border-[#a8d05f]/50 overflow-hidden">
              ...
              <div
                ...
                className={`flex flex-col md:flex-row ${
                  reduceMotion ? "demo-static" : ""
                }`}
              >
                {/* Slides pane */}
                <div className="flex-1 min-w-0 bg-[#f1f3f4] min-h-[500px] flex flex-col">
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                    {split ? (
                      ...
                      <ConceptSlide isVisible={isVisible} />
                    )}
                  </div>
                  ...
                  <div className="bg-white border-t border-gray-200 px-4 py-1.5 flex items-center justify-center gap-3">
                    ...
                    <span className={split ? "demo-arrow-ping" : ""}>
                      <ChevronRightIcon />
                    </span>
                  </div>
                </div>

                {/* Editor pane — slides in when the deck reaches a coding question */}
                <div
                  className={`demo-editor bg-[#1e1e1e] border-t md:border-t-0 md:border-l border-[#a8d05f]/30 ${
                    split ? "demo-open" : ""
                  }`}
                >
                  <div className="flex flex-col md:min-h-[500px] h-full">
                      ...
                      <div className="space-y-1">
                        {[
                          ...
                          <div key={i} className="flex">
                            ...
                            <span className={`code-line ${isVisible && split ? cls : ""}`}>
                              {content}
                            </span>
                          </div>
                        ))}
                      </div>
                      <div className={`code-line ${isVisible && split ? "typing-cursor" : ""}`}>
                    ...
                    <div className="flex-[3] border-t border-[#3e3e42] bg-[#181818] p-4">
                      ...
                      <div className={`font-mono text-sm text-[#a8d05f] ${isVisible && split ? "typing-output" : "opacity-0"}`}>
                        Hello, Student!
                      </div>
                    </div>
                  </div>
...
function ConceptSlide({ isVisible }: { isVisible: boolean }) {
  return (
    <div className="mx-auto my-2 aspect-video w-full max-w-xl md:max-w-2xl bg-white rounded border border-gray-300 shadow-md overflow-hidden flex flex-col p-4 md:p-6">
      <div className={isVisible ? "slide-reveal-0" : "opacity-0"}>
        <h3 className="text-base md:text-xl font-bold text-[#1a1a1a] mb-1">
          Python Functions
        </h3>
      ...
      <div className="space-y-2 flex-1">
        {[
          ...
          <div key={num} className={`flex items-start gap-2 ${isVisible ? delay : "opacity-0"}`}>
            ...
            <p className="text-[11px] md:text-sm text-gray-700 leading-relaxed">{text}</p>
          </div>
        ))}
      </div>
...
function QuestionSlide() {
  return (
    <div className="mx-4 my-2 aspect-video bg-white rounded border border-gray-300 shadow-md overflow-hidden flex flex-col p-4">
      <div className="slide-reveal-0">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#6b8f2b] px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide text-white mb-2">
          <svg className="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M9.4 16.6L4.8 12l4.6-4.6L8 6l-6 6 6 6 1.4-1.4zm5.2 0l4.6-4.6-4.6-4.6L16 6l6 6-6 6-1.4-1.4z" />
          </svg>
          Coding Question
        </span>
        <h3 className="text-base font-bold text-[#1a1a1a] mb-1">Your Turn!</h3>
        <div className="w-10 h-0.5 bg-gradient-to-r from-[#6b8f2b] to-[#a8d05f] rounded-full mb-3" />
      </div>
      <div className="flex-1 space-y-2">
        <p className="text-[11px] text-gray-700 leading-relaxed slide-reveal-1">
          Write a function{" "}
        ...
        <div className="flex items-start gap-2 slide-reveal-2">
          <div className="min-w-[18px] h-[18px] rounded-full bg-[#6b8f2b] flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0 mt-0.5">
            1
          </div>
        ...
        <div className="flex items-start gap-2 slide-reveal-3">
          <div className="min-w-[18px] h-[18px] rounded-full bg-[#6b8f2b] flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0 mt-0.5">
            2
          </div>
          <p className="text-[11px] text-gray-700 leading-relaxed">
